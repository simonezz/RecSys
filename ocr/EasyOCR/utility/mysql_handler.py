...
class MysqlHandler:
    ...
    def __init__(self, username, passwd, hostname=DEFAULT_HOST, port=DEFAULT_PORT,
                 database=None, logger=None, show_=False):
    ...
    def connect_db(self):
        try:
            self.logger.info(" # try connect db : {}:{}. db:{}".format(self.hostname, self.port, self.db_name))
        ...
        for idx in range(3):
            try:
                ...
                for con_idx in range(3):
                    if self.connect_db():
                        break
                    self.logger.error('get_cursor. failed connection. try again. {}'.format(con_idx))
                    time.sleep(1)
    ...
    def insert_csv_file_into_table(self, table_name, csv_fname, row_num=-1):
        ...
        try:
            ...
            self.print_cursor(cursor)
            self.logger.info("")
            self.logger.info(" # insert_csv_file_into_table: {:d} was inserted.".format(cursor.rowcount))
            cursor.close()
        except Exception as e:
    ...
    def update_columns_into_row(self,
        ...
        try:
            ...
            cursor.execute(sql)
            self.connection.commit()
            self.logger.info(" # Update columns into row : table:{}. {:d} record(s) affected.".format(table_name,
                                                                                                      cursor.rowcount))
            cursor.close()
        except Exception as e:
    ...
    def log_exception(self, msg, e):
        self.logger.error(msg + '. Exception: {}'.format(e) + "\n" + traceback.format_exc())
