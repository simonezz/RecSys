...
def split_area_by_condition(img, roi_ratio=0.2, enable_=True, mode='line_detection',
                            ...
                            pause_sec=-1,
                            save_img_=False,
                            save_fpath=None, logger=utils.get_stdout_logger()):
    ...
    if mode == 'contour_detection':
        ...
        split_, line_pts = check_lines_in_img(crop_img, line_detection_algorithm, get_line_=True,
                                              pause_sec=pause_sec, save_img_=save_img_, save_fpath=save_fpath)
        area_pts = line_pts
    ...
    if split_ is False:
        ...
        logger.info(" [SPLIT-AREA] # {:d} boundary lines detected.".format(len(area_pts)))
...
def check_contours_in_img(img,
    ...
    if save_img_:
        ...
        contour_img_fname = os.path.join(dir_name, 'split_area', 'contour_' + file_name + '.jpg')
...
def check_lines_in_img(img, algorithm='HoughLineTransform', get_line_=False,
    ...
    if algorithm == 'HoughLineTransform':
        ...
        for line in lines:
            ...
            if len(x) == 2:
                img_lines = cv2.line(img_rgb, (int(x[0]), int(y[0])), (int(x[1]), int(y[1])), utils.RED, 4)
    ...
    if save_img_:
        dir_name, file_name, _ = utils.split_fname(save_fpath)
        line_img_fname = os.path.join(dir_name, 'split_area', 'line_' + file_name + '.jpg')
...
def derotate_image(img,
    ...
    if check_time_:
        logger.info(" [DEROTATION] # elapsed time : {:.3f} sec".format(float(time.time() - start_time)))

    utils.imshow(np.concatenate((img, rot_img), axis=1), pause_sec=pause_sec, desc="de-rotation")

    if save_img_:
        dir_name, file_name, _ = utils.split_fname(save_fpath)
        rot_img_fname = os.path.join(dir_name, 'derotate', 'derot_' + file_name + '.jpg')
