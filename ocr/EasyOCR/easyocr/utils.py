...
def group_text_box_by_ths(ocr_results, ycenter_ths=0.5, height_ths=0.5, width_ths=1.0):
    ...
    sort_results = swap_text_positions(ocr_results, axis=0, y_margin=40)  # axis=0(row), 1(col)
    ...
    for i, (box, text, score) in enumerate(sort_results):
        ...
        if len(new_box) == 0:
            ...
            # comparable height and comparable y_center level up to ths*height
            if (abs(np.mean(b_height) - (box[3]-box[2])) < height_ths * np.mean(b_height)) and (
                    abs(np.mean(b_ycenter) - (0.5*(box[2]+box[3]))) < ycenter_ths * np.mean(b_height)):
...
def swap_text_positions(text_infos, axis=0, y_margin=40):
    if axis == 0:        
        for i in range(0, len(text_infos) - 1):
            ...
            c_x, next_c_x =  int((text_infos[i][0][0][0]+text_infos[i][0][1][0])/2), int((text_infos[i+1][0][0][0]+text_infos[i+1][0][1][0])/2)  # center x
            ...
            if (y_diff_) and (x_center_):
                text_infos[i], text_infos[i+1] = text_infos[i+1], text_infos[i] # swap row index
